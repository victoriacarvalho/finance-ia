...
// Relatório fictício (caso não consiga acessar a API do OpenAI)
const DUMMY_REPORT =
...
export const generateAiReport = async ({ month }: GenerateAiReportSchema) => {
  ...
  // Se a chave da API não estiver configurada, retornamos o relatório fictício
  ...
  // Gerar o conteúdo para enviar para o OpenAI
  const content = `Gere um relatório com insights sobre as minhas finanças, com dicas e orientações de como melhorar minha vida financeira. Os relatórios devem ser formatados e cada insight deve ser marcado em negrito e com quebra de linha. As transações estão divididas por ponto e vírgula. A estrutura de cada uma é {DATA}-{TIPO}-{VALOR}-{CATEGORIA}. São elas:
  ${transactions
    .map(
      (transaction) =>
  ...
  try {
    const completion = await openAi.chat.completions.create({
      model: "gpt-4",  // ou "gpt-3.5-turbo" para maior compatibilidade
      messages: [
        {
          role: "system",
          content:
    ...
    // Retorna o conteúdo gerado pelo OpenAI
